	...
	Header() {
		return (
			<View className="mt-4 mb-7">
				...
				>
					RENT
					<span className="text-secondary-500 font-light hover:!text-gray-300">
						IFUL
					</span>
				</Heading>
			...
			return (
				<View className="text-center mt-4">
					<p className="text-muted-foreground">
						Don&apos;t have an account? <AuthLink type="sign-up" />
					</p>
				</View>
			);
		},
	...
	SignUp: {
		...
		Footer() {
			return (
				<View className="text-center mt-4">
					<p className="text-muted-foreground">
						Already have an account? <AuthLink type="sign-in" />
					</p>
				</View>
			);
		},
...
function AuthLink({ type }: { type: "sign-in" | "sign-up" }) {
	const { toSignIn, toSignUp } = useAuthenticator();

	return (
		<Button
			variant="link"
			onClick={type === "sign-in" ? toSignIn : toSignUp}
			className="px-2"
		>
			{type === "sign-in" ? "Sign in" : "Sign up here"}
		</Button>
	);
}
